	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			...
			'lideresgestion' => "`lideresgestion`.`LLAVE` as 'LLAVE', `lideresgestion`.`CODGESTION` as 'CODGESTION', IF(    CHAR_LENGTH(`lideres1`.`CEDULA`) || CHAR_LENGTH(`lideres1`.`NOMBRE`), CONCAT_WS('',   `lideres1`.`CEDULA`, ' - ', `lideres1`.`NOMBRE`), '') as 'CEDULA', IF(    CHAR_LENGTH(`lideres2`.`CELULAR`) || CHAR_LENGTH(`lideres2`.`DIRECCION`), CONCAT_WS('',   `lideres2`.`CELULAR`, ' - ', `lideres2`.`DIRECCION`), '') as 'CELULAR', `lideresgestion`.`OBSERVACIONES` as 'OBSERVACIONES', `lideresgestion`.`ESTADO` as 'ESTADO'",
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'gestion' => "`gestion` ",
			'lideresgestion' => "`lideresgestion` LEFT JOIN `lideres` as lideres1 ON `lideres1`.`CEDULA`=`lideresgestion`.`CEDULA` LEFT JOIN `lideres` as lideres2 ON `lideres2`.`CEDULA`=`lideresgestion`.`CELULAR` ",
