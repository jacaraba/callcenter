	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			...
			'lideresgestion' => "`lideresgestion`.`LLAVE` as 'LLAVE', IF(    CHAR_LENGTH(`gestion1`.`CODGESTION`) || CHAR_LENGTH(`gestion1`.`DESGESTION`), CONCAT_WS('',   `gestion1`.`CODGESTION`, ' - ', `gestion1`.`DESGESTION`), '') as 'CODGESTION', IF(    CHAR_LENGTH(`lideres1`.`CEDULA`) || CHAR_LENGTH(`lideres1`.`NOMBRE`), CONCAT_WS('',   `lideres1`.`CEDULA`, ' - ', `lideres1`.`NOMBRE`), '') as 'CEDULA', IF(    CHAR_LENGTH(`lideres2`.`CELULAR`) || CHAR_LENGTH(`lideres2`.`NOMBRE`), CONCAT_WS('',   `lideres2`.`CELULAR`, ' - ', `lideres2`.`NOMBRE`), '') as 'CELULAR', `lideresgestion`.`OBSERVACIONES` as 'OBSERVACIONES', `lideresgestion`.`ESTADO` as 'ESTADO'",
