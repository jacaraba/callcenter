<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'gestion' => "`gestion`.`CODGESTION` as 'CODGESTION', `gestion`.`DESGESTION` as 'DESGESTION'",
			'lideresgestion' => "`lideresgestion`.`LLAVE` as 'LLAVE', if(`lideresgestion`.`HORA`,date_format(`lideresgestion`.`HORA`,'%m/%d/%Y %h:%i %p'),'') as 'HORA', IF(    CHAR_LENGTH(`gestion1`.`CODGESTION`) || CHAR_LENGTH(`gestion1`.`DESGESTION`), CONCAT_WS('',   `gestion1`.`CODGESTION`, ' - ', `gestion1`.`DESGESTION`), '') as 'CODGESTION', IF(    CHAR_LENGTH(`lideres1`.`CEDULA`) || CHAR_LENGTH(`lideres1`.`NOMBRE`), CONCAT_WS('',   `lideres1`.`CEDULA`, ' - ', `lideres1`.`NOMBRE`), '') as 'CEDULA', IF(    CHAR_LENGTH(`lideres2`.`CELULAR`) || CHAR_LENGTH(`lideres2`.`NOMBRE`), CONCAT_WS('',   `lideres2`.`CELULAR`, ' - ', `lideres2`.`NOMBRE`), '') as 'CELULAR', `lideresgestion`.`OBSERVACIONES` as 'OBSERVACIONES', `lideresgestion`.`ESTADO` as 'ESTADO'",
			'lideres' => "`lideres`.`LIDER` as 'LIDER', `lideres`.`CEDULA` as 'CEDULA', `lideres`.`NOMBRE` as 'NOMBRE', IF(    CHAR_LENGTH(`divpol20221`.`PUESTO`) || CHAR_LENGTH(`divpol20221`.`nompue`), CONCAT_WS('',   `divpol20221`.`PUESTO`, '-', `divpol20221`.`nompue`), '') as 'PUESTO', `lideres`.`CELULAR` as 'CELULAR', `lideres`.`DIRECCION` as 'DIRECCION', `lideres`.`CORREO` as 'CORREO', `lideres`.`OBSERVACIONES` as 'OBSERVACIONES', `lideres`.`ESTADO` as 'ESTADO'",
			'amigos' => "`amigos`.`LLAVE` as 'LLAVE', `amigos`.`ESLIDER` as 'ESLIDER', IF(    CHAR_LENGTH(`lideres1`.`CEDULA`) || CHAR_LENGTH(`lideres1`.`NOMBRE`), CONCAT_WS('',   `lideres1`.`CEDULA`, ' - ', `lideres1`.`NOMBRE`), '') as 'LIDER', `amigos`.`CEDULA` as 'CEDULA', `amigos`.`NOMBRE` as 'NOMBRE', IF(    CHAR_LENGTH(`divpol20221`.`PUESTO`) || CHAR_LENGTH(`divpol20221`.`nompue`), CONCAT_WS('',   `divpol20221`.`PUESTO`, ' - ', `divpol20221`.`nompue`), '') as 'PUESTO', `amigos`.`NOMPUESTO` as 'NOMPUESTO', `amigos`.`MESA` as 'MESA', `amigos`.`CELULAR` as 'CELULAR', `amigos`.`DIRECCION` as 'DIRECCION', `amigos`.`CORREO` as 'CORREO', `amigos`.`OBSERVACIONES` as 'OBSERVACIONES', `amigos`.`ESTADO` as 'ESTADO'",
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'gestion' => "`gestion` ",
			'lideresgestion' => "`lideresgestion` LEFT JOIN `gestion` as gestion1 ON `gestion1`.`CODGESTION`=`lideresgestion`.`CODGESTION` LEFT JOIN `lideres` as lideres1 ON `lideres1`.`CEDULA`=`lideresgestion`.`CEDULA` LEFT JOIN `lideres` as lideres2 ON `lideres2`.`CEDULA`=`lideresgestion`.`CELULAR` ",
			'lideres' => "`lideres` LEFT JOIN `divpol2022` as divpol20221 ON `divpol20221`.`PUESTO`=`lideres`.`PUESTO` ",
			'amigos' => "`amigos` LEFT JOIN `lideres` as lideres1 ON `lideres1`.`CEDULA`=`amigos`.`LIDER` LEFT JOIN `divpol2022` as divpol20221 ON `divpol20221`.`PUESTO`=`amigos`.`PUESTO` ",
