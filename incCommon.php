	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			...
			'lideresgestion' => "`lideresgestion`.`LLAVE` as 'LLAVE', IF(    CHAR_LENGTH(`gestion1`.`CODGESTION`) || CHAR_LENGTH(`gestion1`.`DESGESTION`), CONCAT_WS('',   `gestion1`.`CODGESTION`, ' - ', `gestion1`.`DESGESTION`), '') as 'CODGESTION', IF(    CHAR_LENGTH(`lideres1`.`CEDULA`) || CHAR_LENGTH(`lideres1`.`NOMBRE`), CONCAT_WS('',   `lideres1`.`CEDULA`, ' - ', `lideres1`.`NOMBRE`), '') as 'CEDULA', `lideresgestion`.`CELULAR` as 'CELULAR', `lideresgestion`.`OBSERVACIONES` as 'OBSERVACIONES', `lideresgestion`.`ESTADO` as 'ESTADO'",
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'gestion' => "`gestion` ",
			'lideresgestion' => "`lideresgestion` LEFT JOIN `gestion` as gestion1 ON `gestion1`.`CODGESTION`=`lideresgestion`.`CODGESTION` LEFT JOIN `lideres` as lideres1 ON `lideres1`.`CEDULA`=`lideresgestion`.`CEDULA` ",
